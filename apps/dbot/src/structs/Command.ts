...
export abstract class Command {
	...
	/**
	 * Helper to send an embed, handling deferred or replied interactions automatically
	 */
	async sendEmbed(interaction: ChatInputCommandInteraction, embed: EmbedBuilder, ephemeral = false): Promise<any> {
	...
	/**
	 * Helper to reply with a formatted success embed
	 */
	async replySuccess(interaction: ChatInputCommandInteraction, title: string, description: string, ephemeral = false): Promise<any> {
		const embed = this.client.createEmbed()
			.setTitle(`✅  ${title}`)
			.setDescription(description)
			.setColor('#00FF88');
		return this.sendEmbed(interaction, embed, ephemeral);
	}

	/**
	 * Helper to reply with a formatted error embed
	 */
	async replyError(interaction: ChatInputCommandInteraction, title: string, description: string, ephemeral = true): Promise<any> {
		const embed = this.client.createEmbed()
			.setTitle(`❌  ${title}`)
			.setDescription(description)
			.setColor('#FF4444');
