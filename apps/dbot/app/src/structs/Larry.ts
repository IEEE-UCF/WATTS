...
class Larry extends Client {
			...
			for (const filePath of commandFiles) {
				try {
					...
					if (!command?.isCommand) {
						this.logger.fail(`${path.basename(filePath)} does not export a valid Command`);
						continue;
					}
	...
	private async registerSlashCommands(commands: any[]): Promise<void> {
		try {
			...
			const guildId = this.config.servers.main.id;

			if (this.config.debug) {
				this.logger.log('Started refreshing application (/) commands.');
			}

			// Register guild-specific commands for instant updates during dev/testing
			if (guildId && guildId.trim() !== '') {
				// Clear old global commands to eliminate duplicate slash command entries in Discord UI
				await rest.put(
					Routes.applicationCommands(this.getClientId()),
					{ body: [] },
				);
				await rest.put(
					Routes.applicationGuildCommands(this.getClientId(), guildId),
					{ body: commands },
				);
				this.logger.success(`Cleared old global commands & registered guild commands instantly for server ${guildId}.`);
			} else {
