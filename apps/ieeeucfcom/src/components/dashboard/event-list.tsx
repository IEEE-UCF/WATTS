...
export const EventList = () => {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto mt-8 rounded-xl bg-black shadow-sm shadow-[0_0_20px_rgba(250,204,21,0.5)] text-card-foreground flex flex-col p-6 ">
      <CardTitle className="text-2xl font-[subheading-font] mb-6 text-white">
        Events
      </CardTitle>
      <CardContent>
        <div className="w-full max-w-4xl mx-auto">
          <ScrollArea className={`${events.length > 3 ? "h-80" : ""}`}>
            {/* <Separator className="divide-y divide-gray-200"> */}
            {events.length === 0 ? (
              ...
              events.map((event) => (
                <Card
                  key={event.id}
                  className="mb-2 bg-[var(--ieee-dark-grey)] rounded-lg shadow-md p-2"
                >
