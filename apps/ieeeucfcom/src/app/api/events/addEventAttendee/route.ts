	...
	try {
		...
		if (!eventId || !discordId) {
			return NextResponse.json(
				{ success: false, error: 'eventId and discordId are required' },
				{ status: 400 },
			);
		}
		...
		if (!member.active) {
			return NextResponse.json(
				{ success: false, error: 'Member is not active' },
				{ status: 403 },
			);
		}
		...
		const [existingAttendee] = await db
			.select()
			.from(EventAttendees)
			.where(and(eq(EventAttendees.eventId, eventId), eq(EventAttendees.memberId, member.id)))
			.limit(1);
		if (existingAttendee) {
			return NextResponse.json(
		...
		if (event.requiresDues && !member.duesPaid) {
			return NextResponse.json(
				{ success: false, error: 'Dues payment required for this event' },
				{ status: 402 },
			);
		}
		...
		return NextResponse.json(
			{ success: false, error: 'Failed to add event attendee' },
			{ status: 500 },
		);
	}
}
